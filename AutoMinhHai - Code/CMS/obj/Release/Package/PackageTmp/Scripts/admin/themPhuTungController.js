myApp.controller("themPhuTungController", ['$scope', '$http', '$window', '$location', '$filter', 'Alias', 'MenuMultiLevel', 'Url', function ($scope, $http, $window, $location, $filter, Alias, MenuMultiLevel, Url) {
    $scope.product = {};
    $scope.categories = [];
    $scope.category = {};
    ...
    $scope.saveProduct = function () {
        ...
        if ($scope.currentIdProduct) {
            ...
            .success(function () {
                toastr.success('Thành công', 'Lưu phụ tùng');
            })
            .error(function () {
                toastr.error('Thất bại', 'Thêm phụ tùng')
            });
        } else {
            $http.post('/API/ProductsAPI/', $scope.product)
            .success(function () {
                toastr.success('Thành công', 'Thêm phụ tùng');
                $window.location.href = '/Admin/Products/PhuTung';
            })
            .error(function () {
                toastr.error('Thất bại', 'Thêm phụ tùng');
            });
    ...
    $scope.saveProductAndExit = function () {
        ...
        if ($scope.currentIdProduct) {
            ...
            .error(function () {
                toastr.error('Thất bại', 'Lưu phụ tùng');
            });
        } else {
            $http.post('/API/ProductsAPI/', $scope.product)
            .success(function () {
    ...
    $scope.saveProductAndNew = function () {
        ...
        if ($scope.currentIdProduct) {
            ...
            $http.put('/API/ProductsAPI/' + $scope.product.idProduct, $scope.product)
            .success(function () {
                $window.location.href = '/Admin/Products/CreatePhuTung';
            })
            .error(function () {
                toastr.error('Thất bại', 'Lưu phụ tùng')
            });
