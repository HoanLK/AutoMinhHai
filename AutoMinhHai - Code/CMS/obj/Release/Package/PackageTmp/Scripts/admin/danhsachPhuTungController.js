myApp.controller("danhsachPhuTungController", ['$scope', '$http', '$window', 'CategoryProduct', 'MenuMultiLevel', 'uiGridConstants', function ($scope, $http, $window, CategoryProduct, MenuMultiLevel, uiGridConstants) {
    $scope.product = {};
    $scope.products = [];
    $scope.gridOptions = {};
    ...
    $scope.EditProduct = function (idProduct) {
        $window.location.href = '/Admin/Products/CreatePhuTung?idProduct=' + idProduct;
    }
    ...
    $scope.DeleteProduct = function (idProduct) {
        ...
        if (confirm("Bạn có chắc chắn muốn xóa?")) {
            //Xóa
            $http.delete('/API/ProductsAPI?att=idCategoryProduct&&value=3' + idProduct)
            .success(function () {
